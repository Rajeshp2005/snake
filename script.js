...
function touchControl(event) {
    ...
    if (touchY < gameRect.height / 3) {
        direction = { x: 0, y: -1 }; // Up
    } else if (touchY > gameRect.height * 2 / 3) {
        direction = { x: 0, y: 1 }; // Down
    } else if (touchX < gameRect.width / 3) {
        direction = { x: -1, y: 0 }; // Left
    } else if (touchX > gameRect.width * 2 / 3) {
...
gameContainer.addEventListener("touchstart",touchControl);
